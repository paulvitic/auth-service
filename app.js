var express = require('express');
var path = require('path');
var session = require('express-session');
var favicon = require('serve-favicon');
var logger = require('morgan');
var cookieParser = require('cookie-parser');
var bodyParser = require('body-parser');
var oauthserver = require('oauth2-server');

var app = express();

// view engine setup
app.set('views', path.join(__dirname, 'views'));
app.set('view engine', 'hbs');

//app.use(favicon(path.join(__dirname, 'public', 'favicon.ico')));
app.use(logger('dev'));
app.use(bodyParser.json());
app.use(bodyParser.urlencoded({ extended: true }));
app.use(cookieParser());
app.use(session({
    secret: process.env.EXPRESS_SESSION_SECRET || 'someSecret',
    resave: false,
    saveUninitialized: true
}));
app.use(express.static(path.join(__dirname, 'public')));

// get oauthserver
app.oauth = oauthserver({
    model: require('./service/model'),
    // supports authorization_code, password, refresh_token, client_credentials and extension grant types.
    grants: ['authorization_code', 'password'],
    debug: true,
    continueAfterResponse: true
});

// grant types constant
app.locals.grantTypes = {
    authorization_code: 'authorization_code',
    password: 'password',
    refresh_token: 'refresh_token',
    client_credentials: 'client_credentials',
    extension: 'extension'
};

// routes
app.use('/login', require('./routes/login'));
app.use('/oauth/token', require('./routes/token')(app.oauth));
app.use('/oauth/authorise', require('./routes/authorise')(app.oauth));
app.use('/', require('./routes/index')(app.oauth));

// catch 404 and forward to error handler
app.use(function(req, res, next) {
    var err = new Error('Not Found');
    err.status = 404;
    next(err);
});

// error handlers
app.use(app.oauth.errorHandler()); // oauth error handler.

// development error handler
// will print stacktrace
if (app.get('env') === 'development') {
    app.use(function(err, req, res, next) {
        res.status(err.status || 500);
        res.render('error', {
            message: err.message,
            error: err
        });
    });
}

// production error handler
// no stacktraces leaked to user
app.use(function(err, req, res, next) {
  res.status(err.status || 500);
  res.render('error', {
    message: err.message,
    error: {}
  });
});

module.exports = app;

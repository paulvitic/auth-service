...
model.getClient = function (clientId, clientSecret, callback) {
    pg.connect(connString, function (err, client, done) {
        ...
        client.query(
            ...
            'FROM oauth_clients ' +
            'WHERE client_id = $1', [clientId], function (err, result) {
...
model.getRefreshToken = function (bearerToken, callback) {
    pg.connect(connString, function (err, client, done) {
        ...
        client.query('SELECT refresh_token, client_id, expires, user_id ' +
...
model.saveAccessToken = function (accessToken, clientId, expires, userId, callback) {
    pg.connect(connString, function (err, client, done) {
        ...
        client.query(
            'INSERT INTO ' +
            'oauth_access_tokens(access_token, client_id, user_id, expires) ' +
...
model.saveRefreshToken = function (refreshToken, clientId, expires, userId, callback) {
    pg.connect(connString, function (err, client, done) {
        ...
        client.query('INSERT INTO ' +
            'oauth_refresh_tokens(refresh_token, client_id, user_id, expires) ' +
            'VALUES ($1, $2, $3, $4)', [refreshToken, clientId, userId, expires],
            function (err) {
                callback(err);
                done();
            }
...
model.getUser = function (username, password, callback) {
    pg.connect(connString, function (err, client, done) {
        ...
        client.query(
            'SELECT id ' +
            'FROM users ' +
            'WHERE username = $1 AND password = $2', [username, password], function (err, result) {
            callback(err, result.rowCount ? result.rows[0] : false);
            done();
        });
    });
...
model.getAuthCode = function (authCode, callback) {
    pg.connect(connString, function (err, client, done) {
        ...
        client.query(
            'SELECT auth_code, client_id, expires, user_id ' +
...
model.saveAuthCode = function (authCode, clientId, expires, userId, callback) {
    pg.connect(connString, function (err, client, done) {
        ...
        client.query('INSERT INTO ' +
            'oauth_auth_codes(auth_code, client_id, user_id, expires) ' +
